        ...
        >
          <div :class="[c.title, headerClasses]">
            <transition-group name="f-modal-title">
              <button
                v-if="left"
                :aria-label="modalBackAriaLabel"
                @click="$emit('left')"
                :class="titleLeftClasses"
                key="left"
                v-bind="left"
              >
                <slot name="left">
                  <svg
                    aria-hidden="true"
                    :class="[c.titleButtonIcon, 'transform rotate-90']"
              ...
              <div :class="titleCenterClasses" key="title" v-bind="titleAttrs">
                <p id="f-modal-title" :class="c.titleText" v-if="title">
                  {{ title }}
                </p>
                <slot name="title" />
              </div>
          ...
          <div
            :class="[c.content, contentClasses]"
            v-if="$slots.default"
            ref="contentEl"
            :id="contentId"
          >
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const titleLeftClasses = computed(() => ({
      [transitions]: true,
      [titleShouldTransition.value ? 'duration-300' : 'duration-1']: true,
      [c.titleButton]: true,
    ...
    const titleCenterClasses = computed(() => ({
      [transitions]: true,
      [titleShouldTransition.value ? 'duration-300' : 'duration-0']: true,
    ...
    const modifyBorderRadius = () => {
      if (modalEl.value?.scrollHeight * 1.02 > innerHeight)
        modalEl.value.style.borderRadius = '0px';
      else modalEl.value.style.borderRadius = null;
    };

    async function setupHandlers() {
      focusLock.on(modalEl.value);
      modalEl.value.addEventListener('transitionend', modifyBorderRadius, {
        passive: true,
      });
    ...
    async function teardownHandlers() {
      ...
      if (modalEl.value) {
        ...
        modalEl.value.removeEventListener('transitionend', modifyBorderRadius, {
          passive: true,
        });
      }
    ...
    const modalBackAriaLabel = i18n._('Back');
    const modalCloseAriaLabel = i18n._('Close');
