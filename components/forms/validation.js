...
export const defaultInvalid = { valid: false, hint: i18n._('Mandatory field') };
...
export const createValidationCollector = () => {
  ...
  const allChildrenValid = computed(() =>
    validationElements.value.every((e) => e.valid),
  );
  const completed = computed(() =>
    validationElements.value.filter((e) => e.required).every((e) => e.valid),
  );
  ...
  watch(childrenShouldValidate, () =>
    validationElements.value.forEach(
      (e) => (e.shouldValidate = childrenShouldValidate.value),
    ),
  );

  const collectionAttrs = reactive({
    registerValidation,
...
export const createValidation = (props) => {
  ...
  const showValidation = computed(
    () => shouldValidate.value || !!validation.value.always,
  );
  const hasErrorMessage = computed(
    () => showValidation.value && !validation.value.valid,
  );
