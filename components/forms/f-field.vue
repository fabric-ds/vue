<template>
  ...
  >
    <component
      ...
      >{{ label
      }}<span v-if="optional" class="pl-8 font-normal text-14 text-gray-500">{{
        optionalHelperText
      }}</span></component
    >
    <slot
...
export default {
  ...
  setup(props, { slots }) {
    ...
    const labelId = computed(
      () => (props.label || slots.label) && props.id + ':label',
    );
    const hintId = computed(() => props.id + ':hint');
    const errorId = computed(() =>
      valueOrUndefined(hasErrorMessage.value, props.id + ':error'),
    );
    const aria = computed(() => ({
    ...
    const wrapperAria = computed(() =>
      valueOrUndefined(isFieldset.value, aria.value),
    );
    const optionalHelperText = i18n._(' (optional)');
